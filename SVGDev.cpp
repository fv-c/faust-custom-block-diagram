...
void SVGDev::rect(double x, double y, double l, double h, const char* color, const char* link)
{
    ...
    if (gGlobal->gShadowBlur) {
        fprintf(fic_repr,
                ...
                "style=\"stroke:none;fill:none;filter:url(#filter);\"/>\n",
                x + 1, y + 1, l, h);
    } else {
        fprintf(fic_repr,
    ...
    // draw the rectangle
    fprintf(fic_repr,
            "<rect x=\"%f\" y=\"%f\" width=\"%f\" height=\"%f\" rx=\"0\" ry=\"0\" style=\"stroke: black;stroke-width: 0.75;fill:%s;fill-opacity:0\"/>\n",
            x, y, l, h, color);
    if (link != 0 && link[0] != 0) {
...
void SVGDev::triangle(double x, double y, double l, double h, const char* color, const char* link, bool leftright)
{
    ...
    fprintf(fic_repr, "<polygon fill=\"%s\" stroke=\"black\" stroke-width=\".4\" points=\"%f,%f %f,%f %f,%f\"/>\n",
            color, x0, y, x1, y + h / 2.0, x0, y + h);
    fprintf(fic_repr, "<circle  fill=\"%s\" stroke=\"black\" stroke-width=\".4\" cx=\"%f\" cy=\"%f\" r=\"%f\"/>\n",
            color, x2, y + h / 2.0, r);
}
...
void SVGDev::text(double x, double y, const char* name, const char* link)
{
    ...
    char name2[256];
    fprintf(fic_repr,
            "<text x=\"%f\" y=\"%f\" font-family=\"Menlo, Monospace\" font-size=\"6\" text-anchor=\"middle\" "
            "fill=\"#000000\">%s</text>\n",
...
{
    char name2[256];
    fprintf(fic_repr, "<text x=\"%f\" y=\"%f\" font-family=\"Menlo, Monospace\" font-size=\"6\">%s</text>\n", x, y + 2,
            xmlcode(name, name2));
}

void SVGDev::markSens(double x, double y, int sens)
{
    int offset = (sens == 1) ? 3 : -3;
    fprintf(fic_repr, "<circle cx=\"%f\" cy=\"%f\" r=\"1\" style=\"stroke: black; stroke-width:0.5; fill: none;\"/>\n", 
		x + offset, y + offset);
}
